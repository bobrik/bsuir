...
public class HyperNothingGUI extends javax.swing.JFrame
{
     ...
     * Search index cache
     */
    private HashMap< String, HashMap< String, Integer > > searchIndex;

    /**
     * String with tokenizer delimiters to split search request and annotation
     * into words
     */
    private String tokenizedDelimiters = "\n\t \r;,.";

    /**
     * Extension of the files with annotations
     */
    private String annotationExtension = ".annotation";
	...
	this.searchIndex = new HashMap< String, HashMap< String, Integer > >();
    ...
    /**
     * Event handler for searchField action performing
     * @param evt
     */
    ...
    /**
     * Perform search by keywords and show the result
     * @param keywords
     */
    public void performSearch(Set<String> keywords)
    {
	...
	Iterator<String> i = searchIndex.keySet().iterator();
	...
	{
	    page = i.next();
	    result = pageSearchResult(page, keywords);
	    if (result > 0)
	    {
		searchResult.put(page, result);
	    }
	}
	...
	{
	    this.insertTag("big_heading", "Nothing found\n");
	} else
	{
	    this.insertTag("big_heading", "Search results\n\n");

	    ArrayList<Map.Entry<String, Integer>> entriesList = new ArrayList<Map.Entry<String, Integer>>(searchResult.entrySet());

	    Collections.sort(entriesList, new Comparator<Map.Entry<String, Integer>>() {
		public int compare(Map.Entry<String, Integer> left, Map.Entry<String, Integer> right)
		{
		    return left.getValue() < right.getValue() ? 1 : -1;
		}
	    });

	    Iterator<Map.Entry<String, Integer>> ri = entriesList.iterator();
	    Map.Entry<String, Integer> resultPage;
	    while (ri.hasNext())
	    {
		resultPage = ri.next();
		this.insertTag("link", resultPage.getKey());
		this.insertTag("", " - "+resultPage.getValue().toString()+"\n");
	    }
	}
	this.insertTag("", "\nGo to the ");
	this.insertTag("link", "index");
    }

    /**
     * Return page relevancy against provided keywords
     * @param page Page name
     * @param keywords Set of keywords to check
     * @return
     */
    public Integer pageSearchResult(String page, Set<String> keywords)
    {
	Integer weight = 0;
	...
	while (i.hasNext())
	{
	    keywordWeight = searchIndex.get(page).get(i.next());
	...
	} else
	{
	    ...
	    {
		if (children[i].endsWith(".annotation"))
		{
		    this.updatePageSearchIndex(children[i]);
		}
	    }
	}
    }
    ...
    /**
     * Update single page search index
     * @param filename Filename to get annotation
     */
    public void updatePageSearchIndex(String filename)
    {
	String annotationContents;
	try
	{
	    annotationContents = this.openFile(filename);
	} catch (Exception e)
	{
	    return;
	}

	HashMap<String, Integer> pageIndex = new HashMap<String, Integer>();

	StringTokenizer st = new StringTokenizer(annotationContents, this.tokenizedDelimiters);
	...
	while (st.hasMoreElements())
	{
	    word   = st.nextToken();
	    weight = 1;
	    if (pageIndex.containsKey(word))
	    {
		weight += pageIndex.get(word);
	    }
	    pageIndex.put(word, weight);
	}

	this.searchIndex.put(filename.substring(0, filename.indexOf(".annotation")), pageIndex);
    }

    /**
     * Event handler for searchButton action performing
     * @param evt
     */
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
	HashSet<String> keywords = new HashSet<String>();

	StringTokenizer st = new StringTokenizer(searchField.getText(), this.tokenizedDelimiters);
	...
	for (int i = 0; i < nodes.size(); ++i) {
	    ...
	    String nodeContents;
	    try {
		nodeContents = openFile(nodes.elementAt(i)+this.extension);
    ...
    /**
     * Change hypertext source location and clear search index
     */
     ...
     * @param name name of the file in the current directory
	...
	try {
	    String raw = this.openFile(name+this.extension);
